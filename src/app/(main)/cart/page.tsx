  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
        ...
        <h2 className="text-3xl font-medium tracking-tighter mb-2 uppercase font-subheading-main">YOUR BAG IS EMPTY</h2>
        ...
        <Link href="/shop">
          <Button size="lg" className="h-12 px-10 gap-3 group font-medium tracking-[0.2em] text-[10px] rounded-xl uppercase">
        ...
        <div className="flex flex-col lg:flex-row gap-16 items-start">
          ...
          <div className="flex-1 space-y-8">
             <div className="flex justify-between items-end border-b border-border pb-6 font-subheading-main">
                <h1 className="text-3xl md:text-3xl font-medium tracking-tighter capitalize">My <span className="text-primary">Bag ({items.length})</span></h1>
                <button 
                  onClick={clearCart}
                  className="text-[9px] font-medium tracking-[0.2em] text-muted-foreground hover:text-destructive flex items-center gap-2 transition-colors uppercase font-subheading"
                >
                ...
                {items.map((item) => (
                  <div key={item.id} className="group relative bg-card border border-border rounded-3xl p-6 flex flex-col md:flex-row items-center gap-8 transition-all hover:shadow-xl hover:border-primary/20">
                     ...
                     <div className="flex-1 space-y-1 text-center md:text-left">
                        <p className="text-primary font-medium text-[8px] uppercase tracking-[0.2em] font-subheading">STAPLES • PREMIUM</p>
                        <h3 className="font-medium text-lg md:text-xl leading-tight uppercase font-subheading-main">{item.title}</h3>
                        ...
                        <div className="flex items-center justify-center md:justify-start gap-4 pt-1">
                           <span className="text-primary font-medium text-xl tracking-tighter">Rs {item.price.toFixed(2)}</span>
                        </div>
                     </div>
                     ...
                     <div className="flex flex-col items-center gap-2">
                        <div className="flex items-center bg-muted/30 border border-border rounded-xl h-10 px-3 gap-4">
                           ...
                           <span className="font-medium text-sm w-6 text-center tabular-nums">{item.qty}</span>
                        ...
                        <p className="text-muted-foreground text-[8px] font-medium uppercase tracking-[0.2em] font-subheading">Quantity</p>
                     </div>

                     <div className="w-full md:w-32 text-center md:text-right space-y-1">
                        <p className="text-muted-foreground text-[8px] font-medium uppercase tracking-[0.2em] font-subheading">Total</p>
                        <p className="font-medium text-xl tracking-tighter font-subheading-main text-primary">Rs {(item.price * item.qty).toFixed(2)}</p>
                     </div>
          ...
          <aside className="w-full lg:w-96 shrink-0">
             <div className="bg-card border border-border rounded-[2.5rem] p-10 space-y-10 sticky top-32 shadow-sm">
                <div className="space-y-4 font-subheading-main">
                   <h2 className="font-medium text-3xl uppercase tracking-tighter leading-none italic">ORDER <span className="text-primary">SUMMARY</span></h2>
                ...
                <div className="space-y-6">
                   <div className="flex justify-between items-center font-medium text-[14px] uppercase tracking-widest font-subheading">
                      ...
                      <span className="font-medium tracking-tighter text-lg">Rs {totalPrice().toFixed(2)}</span>
                   </div>
                   ...
                   <div className="border-t border-border pt-6 flex justify-between items-end font-subheading-main">
                      <span className="font-medium text-2xl uppercase tracking-tighter">Total Price</span>
                      <div className="text-right">
                         <span className="block text-primary font-medium text-4xl tracking-tighter leading-none">Rs {totalPrice().toFixed(2)}</span>
                         <span className="text-muted-foreground text-[8px] font-medium uppercase tracking-[0.2em] italic font-subheading">Includes all taxes</span>
                      </div>
                   </div>
                </div>
                ...
                <div className="space-y-4 pt-4">
                   <Link href="/checkout" className="block">
                      <Button className="w-full h-14 text-[10px] font-medium tracking-[0.3em] rounded-2xl gap-3 group shadow-lg shadow-primary/20 uppercase">
                         CHECKOUT NOW <ArrowRight className="size-4 group-hover:translate-x-1 transition-transform" />
                      </Button>
                   </Link>
                   <Link href="/shop" className="block">
                      <Button variant="outline" className="w-full h-12 text-[9px] font-medium tracking-[0.2em] rounded-xl border-border text-muted-foreground hover:text-primary transition-all uppercase">
                         CONTINUE SHOPPING
                      </Button>
                   </Link>
                </div>
                 ...
                 <div className="flex flex-col items-center gap-6 text-center">
                    <p className="text-[8px] font-medium tracking-[0.2em] text-muted-foreground uppercase font-subheading">Accepted Secure Payments</p>
